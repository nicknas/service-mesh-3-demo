...
* Stack de **observabilidad**: Kiali, OpenTelemetry Collector, MinIO, Tempo y Grafana

El despliegue se realiza mediante **OpenShift GitOps** (Argo CD). Los manifiestos `01`–`12` definen las Applications de Argo CD que sincronizan los charts Helm desde Git.
...
La Application `grafana` debe sincronizarse **después** de `observability`, ya que los datasources de Prometheus y Tempo dependen del stack de monitorización y tracing.
====
...
[source,bash]
----
for f in 0{1..9}-*.yaml 10-minio.yaml 11-observability.yaml 12-grafana.yaml; do
  echo "Aplicando $f..."
  oc apply -f "$f"
done
----
...
* Datasources provisionados: **Prometheus** (Thanos Querier) y **Tempo** (tenant `bookinfo`)
* Dashboard predefinido *Bookinfo / Service Mesh* con las queries de la demo
...
for ns in bookinfo istio-ingress istio-system istio-cni tracing-system grafana-system \
