...
* Stack de **observabilidad**: Kiali, OpenTelemetry Collector, MinIO, Tempo, Loki, Grafana Alloy y Grafana (Dynatrace Operator + fan-out opcional a Dynatrace SaaS)
...
La Application `logging` debe sincronizarse **después** de `loki-operator` y `minio` (LokiStack usa MinIO como backend S3) y **antes** que `grafana`, ya que el datasource de Loki apunta al gateway del LokiStack en `openshift-logging`.
...
[NOTE]
====
Esta comparativa **no incluye OpenShift Logging** (`cluster-logging`): la demo usa solo el **Loki Operator** + `LokiStack` con recolección mediante Grafana Alloy, sin `ClusterLogForwarder`.
====
...
oc get csv -A | grep -E 'servicemesh|kiali|opentelemetry|tempo|loki|cluster-observability|grafana'
----
...
El manifest `13-logging.yaml` despliega recolección y almacenamiento de **logs de aplicación** de Bookinfo mediante el **Loki Operator** de Red Hat (`LokiStack` CR). Grafana Alloy envía los logs al gateway del LokiStack (tenant `application`). No se instala el operador de logging de OpenShift ni `ClusterLogForwarder`; la recolección sigue siendo Alloy, no el pipeline de plataforma.

Chart: `helm-charts/logging` (LokiStack + Alloy). Operador: `helm-charts/loki-operator` (`05-loki-operator.yaml`).
...
Pods Bookinfo (stdout/stderr)
        │
        ▼
Grafana Alloy (logging-system)  ──(API Kubernetes: pods/log)──►  filtra namespace bookinfo
        │                                                          etiquetas: app, version, pod
        ▼  HTTPS + token SA (tenant application)
LokiStack gateway (openshift-logging)  ──►  MinIO S3 (bucket loki, tracing-system)
        │
        ▼
...
| Namespace `logging-system` | Colector Grafana Alloy
| Namespace `openshift-logging` | Instancia `LokiStack` gestionada por el operador
| **LokiStack** (`logging`) | Almacén de logs (modo `openshift-logging`, tamaño `1x.demo`, backend S3 en MinIO)
...
* **No:** `ClusterLogForwarder` ni logs de infraestructura/auditoría de plataforma (solo tenant `application` de demo).
...
. *loki.write*: envía a `https://logging-gateway-http.openshift-logging.svc:8080/api/logs/v1/application/loki/api/v1/push` con token del ServiceAccount y CA del cluster.
...
for ns in bookinfo istio-ingress istio-system istio-cni tracing-system logging-system openshift-logging grafana-system dynatrace-operator \
