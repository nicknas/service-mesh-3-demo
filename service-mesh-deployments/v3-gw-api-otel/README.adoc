...
====== Vista general (ingesta + consulta)
...
│  │  + sidecar   │                │  + sidecar   │                            │
│  └──────┬───────┘                └──────────────┘                            │
│         │ OTLP gRPC :4317 (solo hacia el collector, vía extensionProvider)    │
└─────────┼───────────────────────────────────────────────────────────────────┘
          │
          ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  OpenTelemetryCollector `otel`  (RH OpenTelemetry Operator)         │    │
│  │  Receiver: otlp/grpc :4317                                          │    │
│  │  Pipeline traces: [otlp] → exporters (fan-out en paralelo)          │    │
│  └────────────┬───────────────────────────────────────┬──────────────────┘    │
│               │ OTLP gRPC + Bearer (SA token)         │ OTLP HTTP + Api-Token │
└───────────────┼───────────────────────────────────────┼──────────────────────┘
                │                                       │
                ▼                                       ▼
┌───────────────────────────────┐       ┌───────────────────────────────────┐
│  namespace: tracing-system    │       │  Internet / SaaS                  │
│  ┌─────────────────────────┐  │       │  https://{ENV_ID}.live.dynatrace  │
│  │ TempoStack gateway      │  │       │         .com/api/v2/otlp          │
│  │ :8090 (gRPC OTLP)       │  │       │  Secret: otel-dynatrace-ingest   │
│  └───────────┬─────────────┘  │       │  (istio-system)                   │
│              ▼                 │       └───────────────────────────────────┘
│  ┌─────────────────────────┐  │
│  │ MinIO (S3) — bucket tempo │  │
│  └─────────────────────────┘  │
└───────────────────────────────┘
                │
                │  consulta (read path, independiente de la ingesta)
                ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  Consumidores in-cluster (no pasan por Dynatrace)                             │
│  • Kiali  ──► Tempo internal_url (traces en topology / workload)             │
│  • Grafana ──► datasource Tempo (dashboards Bookinfo)                        │
│  • Consola OpenShift OSSMC ──► Tempo (plugin observabilidad distribuida)     │
└─────────────────────────────────────────────────────────────────────────────┘

                Dynatrace SaaS (consulta separada, UI externa)
                • Distributed traces, Smartscape, alerting corporativo
                • Mismas trazas que Tempo, vía fan-out del collector
----
...
====== Arquitectura simplificada (referencia rápida)

[source]
----
Istio sidecars (OTLP gRPC :4317)
        │
        ▼
OpenTelemetry Collector (istio-system)  ─── pipeline traces (fan-out)
        ├──► otlp/tempo ──► Tempo :8090 ──► MinIO ──► Kiali / Grafana / OSSMC
        └──► otlphttp/dynatrace ──► https://{env-id}.live.dynatrace.com/api/v2/otlp
----
