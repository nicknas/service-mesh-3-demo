...
* Stack de **observabilidad**: Kiali, OpenTelemetry Collector, MinIO, Tempo, Loki, Grafana Alloy y Grafana (fan-out opcional a Dynatrace SaaS)
...
===== Arquitectura recomendada (fan-out)

Istio mantiene un único destino de exportación (`extensionProvider` `otel` → collector en `istio-system`). El **OpenTelemetry Collector** reenvía en paralelo:
...
OpenTelemetry Collector (istio-system)
        ├──► Tempo gateway :8090  ──► MinIO  ──► Kiali / Grafana
...
[NOTE]
====
Dynatrace SaaS acepta OTLP por **HTTP/protobuf**, no por gRPC directo al tenant. El collector del cluster actúa como traductor: recibe gRPC de Istio y exporta a Tempo (gRPC) y a Dynatrace (HTTP). No hace falta instalar el operador **OpenShift Logging** ni el operador **Dynatrace** solo para este fan-out de trazas mesh; basta con un token de ingest y egress HTTPS hacia el tenant SaaS.
====

===== Ejemplo en esta demo (desactivado por defecto)

El subchart `helm-charts/observability/charts/otel-collector` incluye un exporter opcional `otlphttp/dynatrace` en el pipeline `traces`. Por defecto está **desactivado** (`dynatrace.enabled: false`).
...
. Aplicar los values de ejemplo (sustituir `YOUR_ENV_ID`):
...
# Opción B: Argo CD — añadir en la Application 11:
#   source.helm.valueFiles:
#     - values-dynatrace.example.yaml
----

Fichero de referencia: `helm-charts/observability/values-dynatrace.example.yaml`.

Fragmento del collector con fan-out (cuando `dynatrace.enabled: true`):

[source,yaml]
----
exporters:
  otlp/tempo:
    endpoint: tempo-tempo-gateway.tracing-system.svc.cluster.local:8090
    # ... auth Tempo (existente)
  otlphttp/dynatrace:
...
service:
  pipelines:
    traces:
      receivers: [otlp]
      exporters: [debug, otlp/tempo, otlphttp/dynatrace]
----
...
for ns in bookinfo istio-ingress istio-system istio-cni tracing-system logging-system openshift-logging grafana-system \
