...
El despliegue se realiza mediante **OpenShift GitOps** (Argo CD). Los manifiestos `01`–`13` definen las Applications de Argo CD que sincronizan los charts Helm desde Git.
...
La Application `grafana` debe sincronizarse **después** de `observability` y `logging`, ya que los datasources de Prometheus, Tempo y Loki dependen del stack de monitorización, tracing y logs.
====
...
Comprueba el estado de las 13 Applications:
...
* **Sí:** stdout/stderr de los contenedores de aplicación de `productpage`, `reviews`, `details` y `ratings` en el namespace `bookinfo`.
* **No:** logs del sidecar `istio-proxy` ni del contenedor `istio-validation` (filtrados en Alloy).
* **No:** logs de Envoy access log ni del operador de logging de plataforma OpenShift.
...
==== Cambio en Bookinfo (reviews)

Los deployments de `reviews` escribían logs en archivos bajo `/tmp/logs` (`LOG_DIR`), invisibles para cualquier colector de stdout. Se eliminó esa variable de entorno para que las reseñas también escriban a **stdout**, igual que el resto de servicios.
...
. *loki.process*: descarta contenedores `istio-proxy` e `istio-validation`.
...
El manifest `12-grafana.yaml` despliega:

* Namespace `grafana-system`
* Instancia de **Grafana** con persistencia (PVC)
* Ruta OpenShift para acceso externo
* Datasources provisionados: **Prometheus** (Thanos Querier), **Tempo** (tenant `bookinfo`) y **Loki** (`logging-system`)
* Dashboards predefinidos en carpeta *Bookinfo*:
...
Credenciales por defecto (demo): usuario `admin`, contraseña `grafana` (configurable en `helm-charts/grafana/values.yaml`).

Acceso:

[source,bash]
----
oc get route grafana -n grafana-system -o jsonpath='https://{.spec.host}{"\n"}'
----
...
* **En Explore aparece `loki.source.kubernetes.bookinfo`:** logs antiguos ingestados antes del mapeo `service_name ← app`. Usa rango temporal reciente o filtra `{app="productpage"}`.
